...
export default function Transparency() {
  ...
  return (
    <section id="transparency" className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <h2 className="mt-1 text-lg font-black text-[#0a1a35]">{t('tr.title')}</h2>
      <div className="relative mt-4">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
        <input value={q} onChange={e => setQ(e.target.value)} placeholder={t('tr.search')} className="min-h-11 w-full rounded-lg border border-slate-200 bg-[#f8f9fa] pl-9 pr-3 text-sm outline-none focus:border-[#1a73e8]" />
      </div>
      <div className="mt-4 space-y-2">
        ...
        {!found.length && <p className="rounded-xl border border-dashed border-slate-200 p-4 text-center text-sm text-slate-500">{t('tr.none')}</p>}
      </div>
      <a href="https://pda.teresarizal.gov.ph/" target="_blank" rel="noreferrer" className="mt-4 inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-[#0a1a35] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#15294a]">{t('tr.openAll')} <ExternalLink size={14} /></a>
    </section>
  );
}